...
def show_individual_detail(node):
    print("%s (%s)\n\t-성별: %s\n\t-나이: %s\n\t-전공: %s\n\t사용 가능한 언어"
          % (node.get("name"), node.get("ID"), node.get("sex"), node.findtext("age"), node.findtext("major")))
    try:
...
메뉴 입력: '''
    ...
    while True:
        ...
        if option == '1':
            ...
            if search_option == '1':
                for student in note.getiterator("student"):
                    if search_keyword == student.get("ID"):
                        show_individual_detail(student)
            elif search_option == '2':
                node_list_name = []
                for student in note.getiterator("student"):
                    if search_keyword in student.get("name"):
                        node_list_name.append(student)
                if len(node_list_name) == 1:
                    show_individual_detail(node_list_name[0])
                else:
                    for std_node in node_list_name:
                        print("%s (%s %s %s)" % (std_node.get("ID"), std_node.get("name"), std_node.get("sex"), std_node.find("age").text))
            elif search_option == '3':
                node_list_name = []
                for student in note.getiterator("student"):
                    if search_keyword in student.find("age").text:
                        node_list_name.append(student)
                if len(node_list_name) == 1:
                    show_individual_detail(node_list_name[0])
                else:
                ...
                node_list_name = []
                for student in note.getiterator("student"):
                    if search_keyword in student.find("major").text:
                        node_list_name.append(student)
                if len(node_list_name) == 1:
                    show_individual_detail(node_list_name[0])
                else:
                ...
                for student in note.getiterator("student"):
                    for language in student.find("practicable_computer_languages"):
                        if search_keyword == language.get("name"):
                            node_list_name.append(student)
                            break
                if len(node_list_name) == 1:
                    show_individual_detail(node_list_name[0])
                ...
                for student in note.getiterator("student"):
                    for language in student.find("practicable_computer_languages"):
                        if search_keyword == language.find("period").get("value"):
                            node_list_name.append(student)
                            break
                if len(node_list_name) == 1:
                    show_individual_detail(node_list_name[0])
                ...
                for student in note.getiterator("student"):
                    for language in student.find("practicable_computer_languages"):
                        if search_keyword in language.get("level"):
                            node_list_name.append(student)
                            break
                if len(node_list_name) == 1:
                    show_individual_detail(node_list_name[0])
